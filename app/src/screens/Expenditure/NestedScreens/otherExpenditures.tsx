...
export function OtherExpenditures({ navigation }: any) {
  const mockGroup = [
    {
      month: 'MAIO 2024'
    },
  ...
  const handleGetExpenditures = () => {
    getExpenditures({ type: EExpenditureType.others })
      .then((data: any) => {
        setGroupData(data)
      })
      .catch((err) => {
  ...
  const onSubmitModal = async (data) => {
    try {
      ...
      handleMakeExpenditure({
        ...
        expenditureDate: modalData.dateObj,
        type: EExpenditureType.others
      })
  ...
  return (
    <>
      ...
      <Container>
        <TopBarContainer>
          <ExpenditureHeaderLabel>Outras Despesas</ExpenditureHeaderLabel>
        </TopBarContainer>
        <MainContentContainer>
